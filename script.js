    ...
    entries.forEach(entry => {
        ...
        item.innerHTML = `
            <div class="flex items-center justify-between text-[10px] text-white/40 mb-0.5">
                ...
                <span>${formatTime(entry.id)}</span>
            </div>
            <p class="text-xs text-white/70 truncate">${shortText}</p>
        `;

        item.addEventListener('click', () => openHistory(entry.id));
...
function renderHistoryList(entries, selectedId) {
    ...
    entries
        ...
        .forEach(entry => {
            ...
            `;
            btn.addEventListener('click', () => openHistory(entry.id));
...
function openHistory(id) {
    ...
    if (!selected) {
        ...
        selected = entries.slice().sort((a, b) => (a.id < b.id ? 1 : -1))[0];
    }

    switchView('history');
    renderHistoryList(entries, selected.id);
...
function deleteCurrentEntry() {
    ...
    const idx = entries.findIndex(e => e.id === currentHistoryId);
    if (idx === -1) {
    ...
    renderHistoryList(entries, next.id);
...
function openDeleteConfirm() {
    const entries = loadEntries();
    if (!currentHistoryId || !entries.find(e => e.id === currentHistoryId)) {
        showToast("No entry selected.");
        return;
    }
